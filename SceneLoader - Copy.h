...
public:
	...
	static void savePrimitive(vector<Primitives> &allPrimitives,string &path) {
		...
		for (Primitives& p : allPrimitives) {
			...
			myfile << "Name " << p.name << "\n";
			myfile << "vertexDatas ";
			myfile << p.mesh.MinBounds.x << " " << p.mesh.MinBounds.y << " " << p.mesh.MinBounds.z << " " << p.mesh.MaxBounds.x << " " << p.mesh.MaxBounds.y << " " << p.mesh.MaxBounds.z << "\n";
	...
	static void savejsPrimitive(vector<Objects> objects,vector<Primitives>& allPrimitives, string& path, std::vector<Texture>& textures) {
		...
		for (unsigned int i = 0; i < objects.size(); i++) {
			...
			std::cout << finalname << "\n";
			myfile.open(finalname);
			string namee = Tools::getFileName(finalname);
			size_t lastindex = namee.find_last_of(".");
			string rawname = namee.substr(0, lastindex);
			std::cout << rawname << "\n";
			//	myfile << "Name " << p.name << "\n";
			//	myfile << "vertexDatas \n";
		...
		//	myfile << "]);\n";
			...
			myfile << "const texture = new THREE.DataTexture(imageData.data," << "imageData.width" << "," << "imageData.height" << ","<< tformat << ",THREE.UnsignedByteType,THREE.UVMapping"<<");\n";
			myfile << "texture.needsUpdate = true;\n";
			...
			myfile << " geometry.addAttribute('position',new THREE.BufferAttribute(positions, 3));\n";
			myfile << " geometry.addAttribute('normal',new THREE.BufferAttribute(normals, 3));\n";
			myfile << " geometry.addAttribute('uv',new THREE.BufferAttribute(uvs, 2));\n";
	...
	static void saveSceneData(string filePath, bool exportFully,std::vector<Primitives>& allPrimitives,std::vector<Animation>& animations, std::vector<Objects>& objects, std::vector<Texture>& textures, Lights& sceneLights) {
		...
		for (auto& ob : objects) {
			...
			glm::vec3 minBox = allPrimitives[ob.primitiveID].mesh.MinBounds;
			glm::vec3 maxBox = allPrimitives[ob.primitiveID].mesh.MaxBounds;
			myfile << ob.name << " " << ob.position.x << " " << ob.position.y << " " << ob.position.z << " " << ob.rotation.x << " " << ob.rotation.y << " " << ob.rotation.z << " " << ob.scale.x << " " << ob.scale.y << " " << ob.scale.z << " ";
			myfile << minBox.x << " " << minBox.y << " " << minBox.z << " " << maxBox.x << " " << maxBox.y << " " << maxBox.z << " " << ob.primitiveID << " ";
	...
	static void saveTexture(std::vector<Texture>& textures,unsigned int ID, string path) {
		...
		//if(false){
			...
			//}
			if (ext == "jpg") {
				stbi_write_jpg((finalPath + string(".jpg")).c_str(), textures[ID].width, textures[ID].height, numChannels, pixels, 50);
			}
